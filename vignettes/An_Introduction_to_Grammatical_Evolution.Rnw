...
A grammar is a way to describe structure of a sentence in a language. As programs are also considered 
...
In CFG, there are two type of words, refered to as \emph{symbols}: \emph{Terminal} and \emph{Non-terminal}.
...
This is performed by replacing the first non-terminal symbol with the $n$th rule if that symbol.
For example, 
\begin{itemize}
...
The goal of using GE is to automatically generate full programs:
\begin{enumerate}
...
See \url{http://www.grammatical-evolution.org/pubs.html} for a collection of publication in this area.
...
and \code{expr op expr} is not. As it might be a nuisance to convert between the functional form and the operator form,
...
@
Note that \code{gsrule} and \code{grules} are mixed!
...
or even sing the \code{print} function directly:
\begin{verbatim}
R> ge <- GrammaticalEvolution(grammarDef, SymRegFitFunc, 
+    terminationCost = 0.021,
+    monitorFunc=print)
...
<<tidy=FALSE>>=
ruleDef <- list(re = grule(rex(start, rules, end)),
...
The second element, \ntelem{rules}, is recursive and can create collection of rules such as \ntelem{rule}, \ntelem{rule}, \ntelem{rule}.
The \code{.()} allows using a comma inside a grammar.
The last element, \ntelem{rule}, expands to an RE function or character pattern. These include \code{numbers} (\pkg{rex} shorthand for \code{[:digit:]}),
...
The last step is performing a search for a regular expression that minimise the score function.
...
@
The sequence is zero-indexed (the first rule is zero). To see the step by step mapping, use:
<<tidy=FALSE>>=
GrammarMap(c(0, 1, 0, 0, 1, 1, 0, 0), grammarDef, verbose=TRUE)
@

If the length of each gene is insufficient for the phenotype to genotype conversion,
...
The rest of the parameters are the evolutionary optimisation parameters:
\begin{itemize}
	...
	\item As ordinary cross-over operator is considered destructive when
		homologous production rules are not aligned \citep{oneill2003crossover}, such as in recursive grammars,
		\code{GrammaticalEvolution} uses a heuristic algorithm to 
	...
	\item Finally, \code{plapply} is used for parallel processing.

	\item \code{GrammaticalEvolution} automatically filter non-terminal expressions 
		(i.e., expressions that don't yield a terminal expression even after times of \code{wrappings}).
		Therefore the end-user does not need to worry about them in a real-world optimisation scenario.
\end{itemize}
...
See our paper in the bibliography for an example.
