...
It is used to generate programs with syntax defined through a grammar.
...
other related topics refer to Wikipedia:
\begin{itemize}
...
GA can be readily applied to find optimal programs and expressions:
\begin{enumerate}
	\item The solution is selected as an array of integers.
	...
	\item Any evolutionary optimisation technique is applied on the integer array, which is referred to as the \emph{chromosome} in GA.
\end{enumerate}
...
Applications of GE include computational finance, music, and robotic control, along others. 
...
\proglang{R} supports standard regular expression with both POSIX and \proglang{Perl} RE syntax. 
In addition, the
\href{https://github.com/kevinushey/rex}{\pkg{rex} Package} \cite{rex_package} offers a 
functional interface for creating REs in \proglang{R}.
\begin{verbatim}
R> library("rex")
\end{verbatim}

GE can infer a regular expression from examples \cite{bartoli2012automatic}.
In this section, we use \pkg{gramEvol} to find a RE that matches a decimal real number, 
in form of $[\pm]nnn[.nnn]$, where [~] means optional and $nnn$ denotes one or more digits
(i.e., \textbackslash\code{d+} in \proglang{Perl} or \code{[:digit:]} in POSIX).
...
Score of any RE is determined by counting the number of matches and non-matches:
<<tidy=FALSE>>=
...
A grammar is defined using \pkg{rex} RE functions:
<<tidy=FALSE>>=
...
the codon size and therefore the search space. 
...
\begin{itemize}
	...
	\item if \code{optimizer} is set to ``auto", using the information obtained about the grammar 
		...
		homologous production rules are not aligned, such as for cyclic grammars \citep{oneill2003crossover}.
