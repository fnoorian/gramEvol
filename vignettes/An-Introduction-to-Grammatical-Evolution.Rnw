...
GA can be readily applied to find optimal programs:
\begin{enumerate}
...
\begin{enumerate}
\item Rules are defined in string form using \code{gsrule}.
...
Their definition in \pkg{gramEvol} is as follows:
<<tidy=FALSE>>=
CreateGrammar(list(
       assignment = gsrule("A = B", "A = C"),
       comma      = gsrule("A, B", "B, C")))
@
or
<<tidy=FALSE>>=
CreateGrammar(list(
       assignment = grule(.(A = B), .(A = C)),
       comma      = grule(.(A, B), .(B, C))))
@

\section{Generating more than one expression}

\pkg{gramEvol} supports generation of multiple expressions through:
\begin{itemize}
	\item \code{numExpr} in \code{GrammaticalEvolution} to generate more than one \proglang{R} \code{expression}.
	\item \code{EvalExpressions} to evaluate multiple \code{expression}s at once.
\end{itemize}
